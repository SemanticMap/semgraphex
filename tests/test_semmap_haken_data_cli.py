...
def test_m1_cli_prepare_to_run_persists_checked_spectral_and_dynamics_artifacts(tmp_path: Path, capsys) -> None:
    ...
    # The English fixture contains two disconnected two-node components.  This
    # test exercises the M1 spectral workflow, which requires at least three
    # nodes, so keep all English components rather than selecting one 2-node LCC.
...
paths: {{workspace_root: {tmp_path}, data_root: data, cache_root: cache, runs_root: runs}}
dataset: {{source: fixture, language: en, relations: [], min_weight: 1.0, max_nodes: 10, component: all}}
graph: {{directed: false, weight_transform: raw, operator: normalized_adjacency}}
...
""", encoding="utf-8")
    ...
    payload = __import__("json").loads(output_lines[-1])
