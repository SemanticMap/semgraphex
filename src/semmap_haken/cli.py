"""Stable CLI surface for reproducible Haken-coarsening experiments."""
...
def _not_implemented(command: str) -> CommandHandler:
    def handler(_: argparse.Namespace) -> int:
        print(f"semmap-haken {command}: not yet implemented.", file=__import__("sys").stderr)
        return 2
    return handler


def _add_config_command(subparsers: argparse._SubParsersAction[argparse.ArgumentParser], command: str) -> argparse.ArgumentParser:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = _add_config_command(subparsers, "run")
    run.add_argument("--prepared-graph", help="Completed prepare-run graph directory; overrides spectral.prepared_graph_dir.")
    evaluate = subparsers.add_parser("evaluate", help="Validate an existing run as a start-experiment evidence bundle.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return _HANDLERS.get(arguments.command, _not_implemented(arguments.command))(arguments)
...
def _data_handlers() -> None:
    ...
    from .run_contracts import input_file_identity, validate_prepared_graph_contract, validate_start_run
    import yaml
    ...
    def prepare(args: argparse.Namespace) -> int:
        ...
        paths = save_prepared_graph(
            graph,
            run_dir,
            metadata={"parser_report": report.to_dict(), "source_identity": source_identity},
        ...
        manifest = manifest.__class__(**{
            **manifest.to_dict(),
            ...
            "artifacts": {name: {"path": str(path), "sha256": checksums[path.name]} for name, path in paths.items()},
            "cli_replay": True,
    ...
    def run(args: argparse.Namespace) -> int:
        ...
        resolved_config = json.loads(json.dumps(config.resolved))
        resolved_config["spectral"]["prepared_graph_dir"] = str(prepared_graph_dir)

        m2_cpu_reference_override = config.coarsening.enabled and (config.execution.backend != "cpu" or config.execution.dtype != "float64")
        if config.coarsening.enabled:
            compute = ComputeContext.create(
                ...
                reserved_cpu_cores=config.execution.reserved_cpu_cores, threads_per_worker=config.execution.threads_per_worker,
        ...
        print(json.dumps({"execution": compute.telemetry()["execution"], "m2_cpu_reference_override": m2_cpu_reference_override, "preparation_contract": preparation_contract}, sort_keys=True))
        ...
        modes = analyze_normalized_adjacency(
            operator, degrees=degrees, alpha=config.dynamics.alpha, beta=config.dynamics.beta,
        ...
        batch = build_perturbations(
            node_count=operator.shape[0], degrees=degrees, seed=config.dynamics.perturbation_seed,
        ...
        input_checksums = {"prepared_adjacency": hashlib.sha256((prepared_graph_dir / "adjacency.npz").read_bytes()).hexdigest()}
        spectral_paths = save_mode_result(modes, run_dir / "spectral", resolved_config=resolved_config, input_checksums=input_checksums)
        ...
        dynamics = run_linear_dynamics(
            operator, modes, initial_states=batch.initial_states, labels=batch.labels, time_grid=time_grid,
            storage_policy=config.dynamics.storage_policy, max_storage_bytes=config.dynamics.max_storage_mb * 1024 * 1024, compute=compute,
        )
        dynamics_paths = save_dynamics_result(
            dynamics, run_dir / "dynamics", resolved_config=resolved_config,
            input_artifacts={"prepared_graph": str(prepared_graph_dir), "spectral_numeric": hashlib.sha256(spectral_paths["numeric"].read_bytes()).hexdigest()},
        )
        plot_paths = save_m1_plots(run_dir / "plots", modes, dynamics)
        all_paths = {
            **{f"spectral_{key}": value for key, value in spectral_paths.items()},
            **{f"dynamics_{key}": value for key, value in dynamics_paths.items()},
        ...
        if config.coarsening.enabled:
            ...
            if fine_trajectories is None:
                fine_reference = run_linear_dynamics(
                    operator, modes, initial_states=batch.initial_states, labels=batch.labels, time_grid=time_grid,
            ...
            for method in config.coarsening.methods:
                ...
                quotient = build_quotient(
                    graph.adjacency, partition.fine_to_coarse, aggregation=config.coarsening.aggregation,
                    node_ids=graph.node_ids, level=0, parent_run_id=run_id,
                )
                result = evaluate_one_step(
                    graph.adjacency, modes, partition, quotient, fine_trajectories=fine_trajectories,
                    trajectory_labels=batch.labels, time_grid=time_grid, alpha=float(modes.beta_selection.alpha),
                    beta=beta_value, max_r=config.spectral.max_r, symmetry_tolerance=config.spectral.symmetry_tolerance,
                )
                ...
                m2_paths = save_m2_result(
                    ...
                    input_checksums={
                        "prepared_adjacency": input_checksums["prepared_adjacency"],
                        "spectral_numeric": hashlib.sha256(spectral_paths["numeric"].read_bytes()).hexdigest(),
                        "dynamics_numeric": hashlib.sha256(dynamics_paths["numeric"].read_bytes()).hexdigest(),
                    },
        ...
        manifest = RunManifest.create(run_id=run_id, resolved_config=resolved_config, execution_environment="cli")
        manifest = manifest.__class__(**{
            **manifest.to_dict(),
            "checksums": checksums,
            "stages": stages,
            "artifacts": {name: {"path": str(path), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()} for name, path in all_paths.items()},
            ...
            "resumability": {"prepared_graph_dir": str(prepared_graph_dir), "preparation_contract": preparation_contract, "environment_constraints": "requirements/constraints.txt"},
        })
        manifest.write_json(run_dir / "manifest.json")
        (run_dir / "COMPLETED").write_text("complete\n", encoding="utf-8")
        print(json.dumps({"run_id": run_id, "run_dir": str(run_dir), "coarsening": coarsening_summary}, sort_keys=True))
        return 0

    def evaluate(args: argparse.Namespace) -> int:
        summary = validate_start_run(args.run)
    ...
    register_handler("evaluate", evaluate)
